...
local IdentityLoss, parent = torch.class('nn.IdentityLoss', 'nn.Module')

--[[
Module to compute Identity loss in-place.
...
function IdentityLoss:__init(strength, loss_type)
  parent.__init(self)
  self.strength = strength or 0.0001
  self.loss = 0
  self.target = torch.Tensor()
  self.mode = 'none'
  self.isClassificationLayer = -1
  ...
  elseif loss_type == 'CrossEntropy' then
	  self.crit = nn.CrossEntropyCriterion()
	  self.isClassificationLayer = 0	
  else
...
function IdentityLoss:updateOutput(input)
...
function IdentityLoss:updateGradInput(input, gradOutput)
...
function IdentityLoss:setMode(mode)
