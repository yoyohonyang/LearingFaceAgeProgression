...
local function build_conv_block2(dim, padding_type, use_instance_norm)
  local conv_block = nn.Sequential()
  conv_block:add(nn.SpatialConvolution(dim, dim, 3, 3, 2, 2, 1, 1))
  if use_instance_norm == 1 then
    conv_block:add(nn.InstanceNormalization(dim))
  else
    conv_block:add(nn.SpatialBatchNormalization(dim))
  ...
  conv_block:add(nn.SpatialFullConvolution(dim, dim, 3, 3, 2, 2, 1, 1,1,1))
  if use_instance_norm == 1 then
    conv_block:add(nn.InstanceNormalization(dim))
  else
    conv_block:add(nn.SpatialBatchNormalization(dim))
...
local function build_res_block2(dim, padding_type, use_instance_norm)
  local conv_block = build_conv_block2(dim, padding_type, use_instance_norm)
  local res_block = nn.Sequential()
  local concat = nn.ConcatTable()
  concat:add(conv_block)
  concat:add(nn.Identity())
...
local function build_joint_conv_block(prev_dim, next_dim, use_instance_norm)
  local joint_conv_block = nn.Sequential()
  local pp = nn.ParallelTable(2)
  
  
  -- add convolution layer, BN layer and Relu layer to the age/id features
  local vgg_identity_output = nn.Sequential()
  vgg_identity_output:add(nn.SpatialConvolution(prev_dim, prev_dim, 3, 3, 1, 1, 1, 1)) 
  local vgg_age_output = nn.Sequential()
  vgg_age_output:add(nn.SpatialConvolution(prev_dim, prev_dim, 3, 3, 1, 1, 1, 1))
  
  
  if use_instance_norm == 1 then
    vgg_identity_output:add(nn.InstanceNormalization(prev_dim))
	vgg_age_output:add(nn.InstanceNormalization(prev_dim))
  else
    vgg_identity_output:add(nn.SpatialBatchNormalization(prev_dim))
	vgg_age_output:add(nn.SpatialBatchNormalization(prev_dim))
  end
   
  vgg_identity_output:add(nn.ReLU(true))
  vgg_age_output:add(nn.ReLU(true))
  
  --- add another convolution layer to the id features
  vgg_identity_output:add(nn.SpatialConvolution(prev_dim, prev_dim, 3, 3, 1, 1, 1, 1))
  if use_instance_norm == 1 then   
	vgg_identity_output:add(nn.InstanceNormalization(prev_dim))
  else
  ...
  --- add another convolution layer to the age features
  vgg_age_output:add(nn.SpatialConvolution(prev_dim, prev_dim, 3, 3, 1, 1, 1, 1))
  if use_instance_norm == 1 then   
	vgg_age_output:add(nn.InstanceNormalization(prev_dim))
  else
  ...
  ----JoinTable  
  pp:add(vgg_identity_output)
  pp:add(vgg_age_output)
  joint_conv_block:add(pp)
  joint_conv_block:add(nn.JoinTable(1,3))
  joint_conv_block:add(nn.SpatialFullConvolution(prev_dim*2, next_dim, 3, 3, 2, 2, 1, 1, 1, 1))
  
  if use_instance_norm == 1 then
    joint_conv_block:add(nn.InstanceNormalization(next_dim))
  else
    joint_conv_block:add(nn.SpatialBatchNormalization(next_dim))
  end
  joint_conv_block:add(nn.ReLU(true))
  return joint_conv_block
end


function M.build_model(opt)
  local arch = opt.arch:split(',')
  local prev_dim = opt.prev_dim or 3
  ...
  for i, v in ipairs(arch) do
	...
	elseif first_char == 'D' then
      -- Learned upsampling (strided full-convolution, kernel size 4*4)
      next_dim = tonumber(string.sub(v, 2))
      layer = nn.SpatialFullConvolution(prev_dim, next_dim, 4, 4, 2, 2, 1, 1, 0, 0)
	...
	elseif first_char == 'r' then
      -- Residual (non-bottleneck) block
      next_dim = tonumber(string.sub(v, 2))
      layer = build_res_block2(next_dim, opt.padding_type, opt.use_instance_norm)
      needs_bn = false
      needs_relu = false
	elseif first_char == 'J' then
      -- Joint conv block
      next_dim = tonumber(string.sub(v, 2))
      layer = build_joint_conv_block(prev_dim, next_dim, opt.use_instance_norm)
	  needs_bn = false
      needs_relu = false
    end
    model:add(layer)
